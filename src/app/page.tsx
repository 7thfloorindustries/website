...
function ElevatorLoader({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    const progressInterval = setInterval(() => {
      setLoadProgress((prev) => Math.min(prev + Math.random() * 15, 100));
    }, 200);

    const floorTimers: NodeJS.Timeout[] = [];
    for (let i = 1; i <= 7; i++) {
      floorTimers.push(setTimeout(() => setFloor(i), (i - 1) * 300));
    }
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center">
        ...
        <div className="mt-8 w-32 h-[2px] bg-white/10 rounded-full overflow-hidden">
          <motion.div
